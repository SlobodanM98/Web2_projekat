...
export class WorkPlanInstructionsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.workPlanService.getWorkPlanDevice(Number(localStorage.getItem("workPlan"))).subscribe(data=>{
    ...
    this.workPlanService.getWorkPlanInstruction().subscribe(data=>{
      this.workPlanInstructions = new Array<WorkInstruction>();
      this.workPlanService.getWorkPlanInstructionID(Number(localStorage.getItem("workPlan"))).subscribe(ids=>{
        data.forEach(element => {
          ids.forEach(id => {
            if(element.instructionID === id){
              this.workPlanInstructions.push(element);
            }
          });
        });
      });
